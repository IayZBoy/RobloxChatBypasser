...
CMDs[#CMDs + 1] = {NAME = 'unpowerfling / nopowerfling', DESC = 'Disables powerfling'}
CMDs[#CMDs + 1] = {NAME = 'customfling [power]', DESC = 'Basically fling but no spinning'}
CMDs[#CMDs + 1] = {NAME = 'uncustomfling / nocustomfling', DESC = 'Disables customfling'}
CMDs[#CMDs + 1] = {NAME = 'weakfling', DESC = 'Basically fling but no spinning'}
CMDs[#CMDs + 1] = {NAME = 'unweakfling / noweakfling', DESC = 'Disables weakfling'}
...
powerflinging = false
addcmd("powerfling", {}, function(args, speaker)
    execCmd("unpowerfling")
    local humanoid = speaker.Character:FindFirstChildWhichIsA("Humanoid")
    if humanoid then
    ...
    repeat RunService.Heartbeat:Wait()
        ...
        vel = root.Velocity
        root.Velocity = vel * factor + Vector3.new(0, factor, 0)
    ...
    until powerflinging == false
end)

addcmd("unpowerfling", {"nopowerfling"}, function(args, speaker)
    powerflinging = false
    execCmd("unnoclip")
end)

addcmd("togglepowerfling", {}, function(args, speaker)
    execCmd(powerflinging and "unpowerfling" or "powerfling")
end)

customflinging = false
addcmd("customfling", {}, function(args, speaker)
    execCmd("uncustomfling")
    local humanoid = speaker.Character:FindFirstChildWhichIsA("Humanoid")
	getgenv().factor = args[1]
    local factor = getgenv().factor
    if humanoid then
        humanoid.Died:Connect(function()
            execCmd("uncustomfling")
        end)
    ...
    until customflinging == false
end)

addcmd("uncustomfling", {"nocustomfling"}, function(args, speaker)
    customflinging = false
    execCmd("unnoclip")
end)

addcmd("togglecustomfling", {}, function(args, speaker)
    execCmd(customflinging and "uncustomfling" or "customfling")
end)

weakflinging = false
addcmd("weakfling", {}, function(args, speaker)
    execCmd("unweakfling")
    local humanoid = speaker.Character:FindFirstChildWhichIsA("Humanoid")
    if humanoid then
        humanoid.Died:Connect(function()
    ...
    until weakflinging == false
end)

addcmd("unweakfling", {"noweakfling"}, function(args, speaker)
    weakflinging = false
    execCmd("unnoclip")
end)

addcmd("toggleweakfling", {}, function(args, speaker)
    execCmd(weakflinging and "unweakfling" or "weakfling")
end)
...
addcmd("walkfling", {}, function(args, speaker)
    ...
    repeat RunService.Heartbeat:Wait()
        ...
        vel = root.Velocity
        root.Velocity = vel * 1000000000 + Vector3.new(0, 1000000000, 0)
